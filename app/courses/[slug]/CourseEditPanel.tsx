...
function emptyTee(): CourseTee {
  return { name: '', yards: 0, rating: 0, slope: 0 };
}
...
export default function CourseEditPanel({ course }: { course: Course }) {
  ...
  function updateTee(i: number, field: keyof CourseTee, raw: string) {
    setTees(prev => prev.map((t, idx) => {
      ...
      const n = Number(raw);
      return { ...t, [field]: Number.isFinite(n) ? n : t[field] };
    }));
  }
        ...
        <div className="stack">
          ...
          <div>
            ...
            <div className="stack-sm">
              {/* Header row */}
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: 6, padding: '0 4px' }}>
                {['Tee name', 'Yards', 'Rating', 'Slope', ''].map((h, i) => (
              ...
              <button
                onClick={() => setTees(prev => [...prev, emptyTee()])}
